			...
			.pipe(
				mergeMap((startCoords) => {
					...
					const getSnapGrid = () => {
						...
						if (currentResize) {
							if (
								this.resizeSnapGrid.left &&
								currentResize.edges.left
							) {
							...
							if (
								this.resizeSnapGrid.top &&
								currentResize.edges.top
							) {
					...
					return (
						...
						.pipe(
							filter(([previousCoords, newCoords]) => {
								...
								const newGrid: Coordinate = getGrid(
									newCoords,
									snapGrid
								);
						...
						.pipe(
							map(([, newCoords]) => {
								...
								return {
									clientX:
										Math.round(
											newCoords.clientX / snapGrid.x
										) * snapGrid.x,
									clientY:
										Math.round(
											newCoords.clientY / snapGrid.y
										) * snapGrid.y,
								};
							})
						)
		...
		mousedown$
			...
			.subscribe((edges: Edges) => {
				...
				if (this.enableGhostResize) {
					...
					currentResize.clonedNode!.scrollTop = currentResize
						.startingRect.scrollTop as number;
					currentResize.clonedNode!.scrollLeft = currentResize
						.startingRect.scrollLeft as number;
				}
