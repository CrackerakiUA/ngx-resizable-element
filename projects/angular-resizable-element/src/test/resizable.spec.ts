...
describe('resizable directive', () => {
		...
		afterEach(() => {
			...
			domEvents.forEach((event) => {
				...
				if (event.name !== 'mouseup') {
					expect(
						(elm.nextSibling as HTMLElement).style.position
					).to.equal('fixed');
				}
				if (event.style) {
					Object.keys(event.style).forEach((styleKey) => {
						expect(
							(elm.nextSibling as any).style[styleKey]
						).to.equal(event.style?.[styleKey]);
					});
				}
			});
			expect(
	...
	it('should set the cloned elements width and height on the resize start', () => {
		...
		const elm: HTMLElement =
			fixture.componentInstance.resizable.elm.nativeElement;
		const handle =
			fixture.nativeElement.querySelector('.resize-handle-top');
    ...
    `;
      ...
      `,
			...
			domEvents.forEach((event) => {
				...
				const clonedNode: HTMLElement = (
					elm.parentElement as HTMLElement
